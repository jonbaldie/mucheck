...
- [x] Replace `haskell-src-exts` with the GHC API as the sole AST backend: `haskell-src-exts` is a separate implementation that does not track GHC extensions and silently fails to parse valid GHC 9.x code using `LambdaCase`, `TypeFamilies`, `GADTs`, `LinearTypes`, and others; when the parse fails mucheck produces zero mutants with no diagnostic, which is the worst possible failure mode; migrating to `ghc` + `ghc-exactprint >= 1.12` uses the actual GHC 9.12 parser and replaces `prettyPrint` with `exactPrint` to preserve original source layout, fixing misleading column numbers in diffs; progress tracked in sub-items below
  - [x] Add `ghc >= 9.12 && < 9.13` and `ghc-exactprint >= 1.12 && < 1.13` to `MuCheck.cabal`; update `tested-with` to `GHC ==9.12.1`; create feature branch `migrate/ghc-lib-parser`
  - [x] Rewrite `MuOp.hs`: new GHC type aliases (`Expr_`, `Decl_`, `Alt_`, `Stmt_`, `Rhs_`, `GuardedRhs_`); simplify `MuOp` sum type to `E/D/A/S` constructors; replace `Eq`-based `(~~>)` with SrcSpan-based matching + `transferEntryDP` annotation repair; `CanTransfer`/`MuNode` constraint aliases
  - [x] Rewrite `Utils/Syb.hs`: `isSkippedDecl` updated for GHC `HsDecl GhcPs` constructors (`SigD`, `InstD`, `TyClD`, `DerivD`, etc.)
