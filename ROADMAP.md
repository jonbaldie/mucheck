...
- [ ] Consolidate the triple AST traversal per mutation operator into a single pass: in `genMutantsWithExtra` each operator currently traverses the full AST three times — once in `selectXxx` to generate candidate `MuOp` values, once in `relevantOps` to probe whether the op is applicable, and once in `mutatesN`/`once` to apply it; instead, have `once (mkMpMuOp op)` return `Nothing` when the op does not match any node (collapsing the relevance check into the application pass), and move candidate generation to produce `MuOp` values lazily so the generate and apply passes merge; the `todo.md` entry at line 54 describes the same problem
